<template>
  <div id="act_edit_modal" class="modal fade" tabindex="-1" aria-hidden="true" style="display: none;">
    <div class="modal-dialog">
      <div class="modal-content border-0">
        ...
        <form action="" @submit.prevent="editAct" class="p-4 text-dark" v-if="data">
          <div class="mb-3">
            <label for="actName" class="form-label">Act Name</label>
            <input v-model="data.name"
                   type="text" class="form-control" id="actName" placeholder="Enter emploree name">
          </div>
          <div class="mb-3">
            <label for="actComment" class="form-label">Comment</label>
            <textarea v-model="data.comment"
                      class="form-control" id="actComment" rows="3" placeholder="Enter your message"></textarea>
          </div>
          <div class="mb-3">
          ...
          <div class="text-end">
            ...
            <button v-else disabled class="btn btn-success">Save act</button>
          </div>
        </form>
      </div>
    </div>
...
<script>
import {updateAct} from "@/api/customer/CustomerApi.js"
...
export default {
  ...
  methods: {
    async editAct() {
      ...
      if (response) {
        document.querySelector('#act_edit_modal .modal-header .btn-close').click()
        await Swal.fire({
          icon: 'success',

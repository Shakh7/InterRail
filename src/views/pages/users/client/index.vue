...
export default {
  ...
  computed: {
    user: {
      get() {
        return store.state.users_list.filter(user => user.id === this.userId)[0]
      }
    }
  },
  components: {
  ...
  methods: {
    async getClientOrders() {
      let response = await fetch(`${process.env.VUE_APP_ORDER_URL}/order/list/?customer=${this.userId}`)
      let data = await response.json()
      data = data.sort(function (a, b) {
        return parseFloat(b.order_number) - parseFloat(a.order_number);
      });
      this.orders = data
    },
    getAccount(id) {
      return store.state.users_list.filter(user => user.id === id)[0]
    }
  },
  async mounted() {
...
<template>
  ...
  <div class="pt-4 mb-4 mb-lg-3 pb-lg-4">
    <div class="row g-4">
      ...
      <!--end col-->
      <div class="col" v-if="user !== undefined">
        <div class="p-2">
          <h3 class="text-white mb-1">
            {{ user.full_name }}
          </h3>
          <p class="text-white-75">{{ user.role.toUpperCase() }}</p>
      ...
      <div>
        <div class="d-flex">
          ...
          <div class="flex-shrink-0">
            <b-button variant="success"><i class="ri-edit-box-line align-bottom"></i> Edit Profile</b-button>
          </div>
        </div>
        <!-- Tab panes -->
        <div class="tab-content pt-4 text-muted">
                      ...
                      >
                        <swiper-slide v-for="order in orders" :key="order.id">
                          <div class="swiper-slide">
                            <div
                                class="card profile-project-card shadow-none mb-0"
                                :class="order.shipment_status === 'delivered' ? 'profile-project-primary' :
                                        order.shipment_status === 'completed'? 'profile-project-success' : 'profile-project-warning'"
                            >
                                ...
                                <div class="d-flex">
                                  <div class="flex-grow-1 text-muted overflow-hidden">
                                    ...
                                    <p class="text-muted text-truncate mb-0">
                                      Type :
                                      <span class="text-truncate text-dark text-capitalize">
                                  ...
                                  <div class="flex-shrink-0 ms-2">
                                    <div
                                        class="badge fs-10 text-capitalize"
                                        :class="order.shipment_status === 'delivered' ? 'badge-soft-primary' :
                                                order.shipment_status === 'completed' ? 'badge-soft-success' : 'badge-soft-warning'"
                                    >
                                      <span>{{ order.shipment_status }}</span>
                                    </div>
                                  </div>
                                </div>
                                <div class="d-flex mt-4">
                                  <div class="flex-grow-1">
                                    <div class="d-flex align-items-center gap-2">
                                      ...
                                      <div class="avatar-group">
                                        <router-link :to="{name: 'user_profile', params: {id: order.manager}}">
                                          <div class="avatar-group-item">
                                            <div class="avatar-xs">
                                              <div class="avatar-title rounded-circle bg-light text-primary">
                                                {{ getAccount(order.manager).full_name[0] }}
                                              </div>
                                            </div>
                                          </div>
                                        </router-link>
          ...
          <div class="tab-pane fade" id="projects" role="tabpanel">
            <div class="card">
              <div class="card-body">
                <div class="row">
                  <div v-for="order in orders" :key="order" class="col-xxl-3 col-sm-6">
                    <div
                        class="card profile-project-card shadow-none"
                        :class="order.shipment_status === 'delivered' ? 'profile-project-primary' :
                                        order.shipment_status === 'completed'? 'profile-project-success' : 'profile-project-warning'"
                    >
                      <div class="card-body p-4">
                        <div class="d-flex">
                          <div class="flex-grow-1 text-muted overflow-hidden">
                            <h5 class="fs-14 fw-semibold">
                              <a href="#" class="text-dark"
                              >{{ 'Order - ' + order.order_number }}</a
                              >
                            </h5>
                            <p class="text-muted text-truncate mb-0">
                              Type :
                              <span class="fw-medium text-dark text-capitalize"
                              >{{ order.position.replace('_', ' ') }}</span
                              >
                            </p>
                          </div>
                          <div class="flex-shrink-0 ms-2">
                        ...
                        <div class="d-flex mt-4">
                          <div class="flex-grow-1">
                            <div class="d-flex align-items-center gap-2">
                              ...
                              <h6 class="my-0">{{ getAccount(order.manager).full_name }}</h6>
                            </div>
                          </div>
                        </div>
                      </div>
